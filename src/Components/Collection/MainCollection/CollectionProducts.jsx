...
// const CollectionProducts = ({ filter, grid, infiniteScroll, categorySlug, authorSlug, publicationSlug }) => {
...
//   useEffect(() => {
//     categorySlug && publicationSlug && authorSlug && !isRefetching;
//   }, [categorySlug, authorSlug, publicationSlug]);
...
//         )
//       ) : (
//         <div className="load-more-sec">{fetchStatus != "idle" ? <img src={`${ImagePath}/loader.gif`} /> : <a onClick={() => onLoad()}>{t("load_more")}</a>}</div>
//       )}
//     </>
//   );
// };
...
//         page,
//         status: 1,
//         paginate: filter?.paginate ?? 30, // Default to 30 if not set
//         field: filter?.field ?? "created_at",
//         price: filter?.price?.join(",") ?? "",
//         category: categorySlug ? categorySlug : filter?.category?.join(",") || tagParam,
//         brand: filter.brand?.join(",") ?? "",
...
//   const { data, fetchNextPage, isRefetching, isLoading, fetchStatus, refetch } = useInfiniteQuery(
//     {
//     queryKey: ["infiniteScroll", filter, page], // Added page to queryKey
//     queryFn: fetchData,
//     retryOnMount: false,
//     enabled: false,
//     getNextPageParam: ({ page, last_page }) => last_page > page && { page: page + 1 },
...
//   // Reset page and refetch when filter changes
//   useEffect(() => {
//     setPage(1);
//     setInfiniteScrollData([]); // Clear infinite scroll data
//     refetch();
//   }, [filter]);
...
const CollectionProducts = ({ filter, grid, infiniteScroll, categorySlug, authorSlug, publicationSlug, setTotalProducts }) => {
  ...
  const { data, fetchNextPage, isRefetching, isLoading, fetchStatus, refetch } = useInfiniteQuery(
    {
    queryKey: ["infiniteScroll", filter, page],
    queryFn: fetchData,
    retryOnMount: false,
    enabled: false,
    getNextPageParam: ({ page, last_page }) => last_page > page && { page: page + 1 },
