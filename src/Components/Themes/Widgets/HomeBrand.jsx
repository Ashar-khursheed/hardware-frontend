    ...
    <>
      <Container>
        {filteredBrand?.length ? (
                ...
                {filteredBrand?.map((item, index) => (
                  <div key={index}>
                    <Link
                      ...
                      legacyBehavior><span>
                        <a>
                          {item.brand_image?.original_url ? <img src={item.brand_image?.original_url} alt="" className="img-fluid brandimages" /> : <h4>{item?.name}</h4>}
                        </a>
                      </span></Link>
                  </div>
                ))}
          ...
          <NoDataFound customClass="no-data-added" title="no_brand" />
        )}
      </Container>
    </>
  );
